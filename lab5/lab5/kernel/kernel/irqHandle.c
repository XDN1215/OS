...
void syscallOpen(struct StackFrame *sf) {
	...
	if (ret == 0) { // file exist
		...
		if ((sf->edx >> 3) % 2 == 0) { 
			...
			char fa_str[NAME_LENGTH];
			stringCpy(str, fa_str, size+1);
			int fa_ret = readInode(&sBlock, gDesc, &fatherInode, &fatherInodeOffset, fa_str);
			if(fa_ret == -1){
				sf->eax=-1;
				return;
			}
			char* destfilename = str+size+1;
			int alloc_ret = allocInode(&sBlock, gDesc, &fatherInode, fatherInodeOffset, &destInode, \
			&destInodeOffset, destfilename, REGULAR_TYPE);
			if(alloc_ret==-1){
				sf->eax=-1;
				return;
			}
			...
			char* destfilename = str+size+1;
			int alloc_ret = allocInode(&sBlock, gDesc, &fatherInode, fatherInodeOffset, &destInode, \
			&destInodeOffset, destfilename, DIRECTORY_TYPE);
			if(alloc_ret==-1){
				sf->eax=-1;
				return;
			}
...
void syscallWrite(struct StackFrame *sf) {
	...
	if(FCBindex < 0 || FCBindex >= MAX_FILE_NUM){
		sf->eax=-1;
		return;
	}
	if(file[FCBindex].state == 0){
		sf->eax=-1;
		return;
	}
	syscallWriteFile(sf);
...
void syscallReadFile(struct StackFrame *sf) {
	...
	int ret = readBlock(&sBlock, &inode, quotient+j, buffer);
	if(ret==-1){sf->eax=-1;return;}

	MemCpy(buffer+remainder, str+stroff, BLOCK_SIZE-remainder);
	stroff+=BLOCK_SIZE-remainder;
	j++;
	...
	while(stroff < size){
		ret = readBlock(&sBlock, &inode, quotient+j, buffer);
		if(ret==-1){sf->eax=-1;return;}
		MemCpy(buffer, str+stroff, BLOCK_SIZE);
		stroff+=BLOCK_SIZE;
		j++;
	}
...
void syscallRemove(struct StackFrame *sf) {
	...
	if (ret == 0) { // file exist
		...
		if (destInode.type == REGULAR_TYPE) {
			...
			if(size == stringLen(str)-1){//路径是个目录
				sf->eax=-1;
				return;
			}
			char fa_str[NAME_LENGTH];
