...
extern int pcb_ablequeue[MAX_PCB_NUM];
extern int front;
extern int rear;
...
void timerHandle(struct StackFrame *sf){
	...
	for (i = 0; i < MAX_PCB_NUM; i++) {
		if(pcb[i].state == STATE_BLOCKED){
			...
			if(pcb[i].sleepTime == 0){
				...
				if((rear+1)%(MAX_PCB_NUM) != front){
				pcb_ablequeue[rear] = i;
	...
	if(pcb[current].state != STATE_RUNNING || (pcb[current].state == STATE_RUNNING && (++pcb[current].timeCount)>=MAX_TIME_COUNT)){
		if(front != rear){
			next = pcb_ablequeue[front];
		...
		if(pcb[current].state == STATE_RUNNING){
			...
			if(current != 0 && (rear+1)%(MAX_PCB_NUM) != front){
				pcb_ablequeue[rear] = i;
				rear = (rear+1)%(MAX_PCB_NUM);
			}
		}
...
void syscallExec(struct StackFrame *sf) {
	...
	secstart = sf->ecx;
	secnum = sf->edx;
	loadelf(secstart, secnum, (uint32_t)((current+1)*0x100000), &entry);
	sf->eip = entry;
	sf->eax = 0;
}
